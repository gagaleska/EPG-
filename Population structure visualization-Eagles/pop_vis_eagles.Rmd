...
corr_data <- paste("./plink",
                   "--file eagle",                # PLINK binary input files (prefix only)
                   "--export ped",                 # Specify 'ped' as the output format
                   "--allow-extra-chr",
                   "--chr-set 38",
                   "--out eagles_corr")        # Specify output prefix
system(corr_data)
```

```{r}
eagle<-read.PLINK("eagle.raw", ped.file = "eagle.ped", map.file = "eagle.map")
eagle<-gl.compliance.check(eagle)
```
...
```{r}
gl.pcoa.plot(x = eagle, glPca = pca)
```
...
```{r}
gl.pcoa.plot(x = eagle, glPca = pca, zaxis = 3)
...
```{r}
gl.tree.nj(eagle, type = "phylogram")
```
...
```{r}
str <- gl.run.structure(eagle[,1:1000], k.range= 1:6, num.k.rep = 2, exec = "./structure", numreps=200, burnin=100)
```
...
There are four groups in the paper so it is enough that I use these four groups and find coordinates just for those.
Think if the isokation bz distance is significant, if it is not other things influence the diversity and genetic differences. (for example there can be species close geographically, but far genetically)
